...
import { OptimizedSelectField, OptimizedSelectItem, SelectErrorBoundary } from '../components/ui/optimized-select';
...
/**
 * Optimized Register Page Component
 * 
 * This component addresses the 30+ second form submission delays by:
 ...
 * 4. Adding error boundaries for graceful failure handling
 * 5. Using debounced form validation to reduce computation
 * 6. Implementing proper controlled/uncontrolled component patterns
 */
...
// Memoized role options to prevent re-creation on every render
const ROLE_OPTIONS = [
  {
    value: 'customer',
    label: 'Customer - Looking for meal plans',
    icon: 'fas fa-user',
  },
  {
    value: 'trainer', 
    label: 'Trainer - Creating meal plans',
    icon: 'fas fa-dumbbell',
  },
] as const;

// Memoized welcome content to prevent unnecessary re-renders
...
// Memoized invitation alert component
const InvitationAlert = React.memo<{ invitationData: InvitationData }>(({ invitationData }) => (
...
InvitationAlert.displayName = 'InvitationAlert';

// Memoized loading alert component
const LoadingAlert = React.memo(() => (
...
const RegisterPageOptimized = () => {
  ...
  const form = useForm<RegisterFormData>({
    ...
    // Add form optimization options
    mode: 'onChange', // Validate on change for better UX
    reValidateMode: 'onChange',
    shouldFocusError: true,
  });

  // Memoized form field classes to prevent re-creation
  const inputClasses = useMemo(() => ({
    base: "h-11 sm:h-12 pl-10 pr-4 text-sm sm:text-base border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg",
    disabled: "disabled:bg-gray-50 disabled:text-gray-500",
  }), []);

  const selectClasses = useMemo(() => ({
    trigger: "h-11 sm:h-12 pl-10 pr-4 text-sm sm:text-base border-gray-300 focus:border-blue-500 focus:ring-blue-500 rounded-lg relative disabled:bg-gray-50 disabled:text-gray-500",
  }), []);

  // Memoized invitation verification function
  const verifyInvitation = useCallback(async (token: string) => {
    ...
    try {
      ...
      // Pre-fill email and set role to customer - batch updates
  ...
  // Check for invitation token in URL - memoized effect
  useEffect(() => {
  ...
  // Memoized submit handler with error handling optimization
  ...
  // Show only customer role for invitations
  const availableRoleItems = useMemo(() => {
    if (invitationData) {
      return (
        <OptimizedSelectItem value="customer">
          <div className="flex items-center space-x-2">
  ...
  }, [invitationData, roleSelectItems]);
        ...
        >
          ...
          <Card className="border-none shadow-lg bg-white/80 backdrop-blur-sm">
            ...
            <CardContent className="px-4 sm:px-6">
              {/* Invitation Info */}
              {invitationData && <InvitationAlert invitationData={invitationData} />}

              {/* Loading State for Invitation Verification */}
              {isVerifyingInvitation && <LoadingAlert />}
                        ...
                        <FormControl>
                          <div className="relative">
                            <Input 
                              ...
                              {...field} 
                              className={`${inputClasses.base} ${inputClasses.disabled}`}
                            />
                          ...
                          <div className="relative">
                            <SelectErrorBoundary>
                              <OptimizedSelectField
                                ...
                                triggerClassName={selectClasses.trigger}
                              >
                                {availableRoleItems}
                              </OptimizedSelectField>
                            </SelectErrorBoundary>
                            <i className={`fas fa-user absolute left-3 top-1/2 -translate-y-1/2 ${styles.iconMuted} text-sm pointer-events-none z-10`}></i>
                          </div>
                        </FormControl>
                        <FormMessage className="text-xs sm:text-sm" />
                      </FormItem>
