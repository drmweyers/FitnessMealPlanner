...
// MealPlanRating component removed - feature deleted
import {
  User,
  Heart,
  Target,
...
export default function CustomerProfile() {
  ...
  // Rating dialog state removed - feature deleted
  ...
  const handleSaveProfile = () => {
    ...
    if (editForm.fitnessGoals !== (profile?.fitnessGoals?.join(", ") || "")) {
      updateData.fitnessGoals = editForm.fitnessGoals
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s);
    }
    ...
    if (
      ...
      updateData.dietaryRestrictions = editForm.dietaryRestrictions
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s);
    }
    ...
    if (
      editForm.preferredCuisines !==
      (profile?.preferredCuisines?.join(", ") || "")
    ) {
      updateData.preferredCuisines = editForm.preferredCuisines
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s);
    }
  ...
  return (
    <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm sm:text-base text-slate-600">
              Manage your fitness journey and preferences
            </p>
          </div>
        </div>
        <Badge
          variant="secondary"
          className="bg-green-100 text-green-700 border-green-200 text-xs sm:text-sm"
        >
              ...
              {!isEditing ? (
                <>
                      ...
                      <p className="text-sm sm:text-base text-slate-900 truncate">
                        {profile?.activityLevel
                          ? activityLevels
                              .find(
                                (level) =>
                                  level.value === profile.activityLevel,
                              )
                              ?.label.split("(")[0]
                              .trim() || profile.activityLevel
                          : "Not specified"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm sm:text-base text-slate-900">
                        {profile?.weight || "Not specified"} kg
                      </p>
                    </div>
                    <div>
                      <Label className="text-xs sm:text-sm font-medium text-slate-600">
                      ...
                      <p className="text-sm sm:text-base text-slate-900">
                        {profile?.height || "Not specified"} cm
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1 line-clamp-2">
                      Suggestions:{" "}
                      {commonDietaryRestrictions.slice(0, 5).join(", ")}...
                    </p>
                  </div>

                  <div>
                    <Label
                      htmlFor="preferredCuisines"
                      className="text-xs sm:text-sm"
                    >
              ...
              <CardContent className="space-y-3 sm:space-y-4 p-4 sm:p-6">
                {calculateBMI() && (
                  <div className="text-center p-3 sm:p-4 bg-orange-50 rounded-lg">
                    <Scale className="w-6 h-6 sm:w-8 sm:h-8 text-orange-600 mx-auto mb-2" />
                    <div className="text-xl sm:text-2xl font-bold text-slate-900">
                      {calculateBMI()}
                    </div>
              ...
              {stats ? (
                ...
                <div className="text-center py-4 text-slate-500">
                  ...
                  <p className="text-xs text-slate-400">
                    Your trainer will assign plans soon!
                  </p>
                </div>
              )}
            </CardContent>
      ...
      {/* Meal Plan Rating Dialog removed - feature deleted */}
    </div>
  );
}
