...
const Customer = ({ initialTab }: CustomerProps = {}) => {
  ...
  const [selectedMealPlan, setSelectedMealPlan] =
    useState<EnhancedMealPlan | null>(null);
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery<MealPlanResponse, Error>({
  ...
  const filteredMealPlans = useMemo(() => {
    ...
    return mealPlans
      .filter((mealPlan: EnhancedMealPlan) => {
        ...
        if (
          searchTerm &&
          !mealPlan.planName.toLowerCase().includes(searchTerm.toLowerCase())
        ) {
          return false;
        }
        ...
        // Fitness goal filter
        if (
          fitnessGoalFilter !== "all" &&
          mealPlan.fitnessGoal !== fitnessGoalFilter
        ) {
          return false;
        }
  ...
  const stats = useMemo(() => {
    if (summary) {
      return {
        ...
        totalDays: mealPlans.reduce(
          (sum: number, plan: EnhancedMealPlan) => sum + plan.totalDays,
          0,
        ),
    ...
    const activePlans = mealPlans.filter(
      (plan: EnhancedMealPlan) => plan.isActive,
    ).length;
    const totalDays = mealPlans.reduce(
      (sum: number, plan: EnhancedMealPlan) => sum + plan.totalDays,
      0,
    );
    const avgCalories = Math.round(
      mealPlans.reduce(
        (sum: number, plan: EnhancedMealPlan) => sum + plan.dailyCalorieTarget,
        0,
      ) / totalPlans,
    );
    const mostCommonGoal = mealPlans.reduce(
      (acc: Record<string, number>, plan: EnhancedMealPlan) => {
    ...
    const primaryGoal =
      Object.entries(mostCommonGoal).sort(
  ...
  const handleDeleteMealPlan = (mealPlanId: string) => {
    const mealPlan = mealPlans.find(
      (plan: EnhancedMealPlan) => plan.id === mealPlanId,
    );
    if (mealPlan) {
  ...
  return (
    <div className="w-full">
      ...
      <div className="mb-8 lg:mb-12">
        <div className="text-center mb-8">
          ...
          <p className="text-lg sm:text-xl text-slate-600 max-w-2xl mx-auto leading-relaxed">
            Track your progress and discover personalized meal plans crafted by
            expert trainers
          </p>
        ...
        >
          ...
          <TabsContent value="meal-plans">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 xs:gap-3 sm:gap-4 lg:gap-6 mb-6 sm:mb-8">
                <Card className="group hover:shadow-lg transition-all duration-300 border-0 bg-white/70 backdrop-blur-sm hover:bg-white/90">
                  <CardContent className="p-3 sm:p-4 lg:p-6 text-center">
                    ...
                    <div className="text-lg sm:text-xl lg:text-3xl font-bold text-slate-900 mb-1">
                      {stats.totalPlans}
                    </div>
                ...
                <Card className="group hover:shadow-lg transition-all duration-300 border-0 bg-white/70 backdrop-blur-sm hover:bg-white/90">
                  <CardContent className="p-3 sm:p-4 lg:p-6 text-center">
                    ...
                    <div className="text-sm sm:text-base lg:text-xl font-bold text-slate-900 mb-1 truncate">
                      {formatGoalName(stats.primaryGoal)}
                    </div>
                    <div className="text-xs sm:text-sm lg:text-base text-slate-600 font-medium">
                      Primary Goal
                    </div>
            ...
            <div className="mb-6 sm:mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <Card
                  className="group hover:shadow-lg transition-all duration-300 cursor-pointer border-0 bg-gradient-to-br from-orange-50 to-orange-100/50 hover:from-orange-100 hover:to-orange-200/50"
                ...
                {showFilters && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 xs:gap-3 sm:gap-4 pt-2 xs:pt-4 border-t border-slate-200">
                    <div className="space-y-2">
                      ...
                      <Select
                        value={fitnessGoalFilter}
                        onValueChange={setFitnessGoalFilter}
                      >
                ...
                {filteredMealPlans && filteredMealPlans.length > 0 ? (
                  <>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 xs:gap-4 sm:gap-6">
                      {filteredMealPlans.map(
                        (mealPlan: EnhancedMealPlan, index: number) => (
                          <div
                            key={mealPlan.id}
          ...
          <TabsContent value="grocery-list">
            ...
            <GroceryListWrapper activeMealPlan={activeMealPlan} />
          </TabsContent>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !open && setMealPlanToDelete(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="font-semibold">"{deletingPlanName}"</span>? This
              action cannot be undone and will permanently remove this meal plan
              from your account.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
