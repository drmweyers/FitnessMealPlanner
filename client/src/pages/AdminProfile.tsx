            ...
            if (customersData.customers) {
              const customerMealPlans = await Promise.all(
                customersData.customers.map(async (customer: any) => {
                  const mealPlansRes = await apiRequest(
                    "GET",
        ...
        <div className="flex flex-col sm:flex-row sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 mb-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm sm:text-base text-slate-600">
              System administrator dashboard and account settings
            </p>
          </div>
        </div>
        <Badge
            ...
            <CardContent className="flex items-center space-x-6 p-4 sm:p-6">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-slate-600 mb-2">
                  Upload a professional profile image that will be displayed in
                  your header and profile.
                </p>
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
                ...
                <div className="text-center p-3 sm:p-4 bg-slate-50 rounded-lg col-span-2 sm:col-span-1">
                  <ChefHat className="w-6 h-6 sm:w-8 sm:h-8 text-orange-600 mx-auto mb-2" />
                  <div className="text-lg sm:text-2xl font-bold text-slate-900">
            ...
            <CardContent className="p-4 sm:p-6">
              <div className="space-y-4">
                <p className="text-sm text-slate-600">
                  Export all recipe cards from the system to PDF format for
                  administrative purposes and backups.
                </p>
                ...
                {allMealPlans && allMealPlans.length > 0 ? (
                    ...
                    <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                      <div>
                        <div className="font-medium text-slate-900">
                          Export All System Meal Plans
                        </div>
                        <div className="text-sm text-slate-600">
                          {allMealPlans.reduce(
                            (total, entry) => total + entry.mealPlans.length,
                            0,
                          )}{" "}
                          total meal plans across all customers
                        </div>
                      </div>
                      <PDFExportButton
                        mealPlans={allMealPlans.flatMap(
                          (entry) => entry.mealPlans,
                        )}
                        variant="outline"
                        size="sm"
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="max-h-48 overflow-y-auto space-y-1">
                        {allMealPlans.map((entry) => (
                          <div
                            key={`${entry.customer.id}-${entry.trainer.id}`}
                          ...
                          >
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-xs text-slate-600 truncate">
                                Trainer: {entry.trainer.email} •{" "}
                                {entry.mealPlans.length} plan
                                {entry.mealPlans.length !== 1 ? "s" : ""}
                              </div>
                            </div>
                            <PDFExportButton
                  ...
                  <div className="text-center py-6 text-slate-500">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      Meal plans will appear here when trainers create and
