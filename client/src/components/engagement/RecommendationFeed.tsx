...
const RecommendationFeed = memo(
  ({ className, onRecipeClick }: RecommendationFeedProps) => {
    const [activeType, setActiveType] = useState<
      "personalized" | "similar" | "trending" | "new"
    >("personalized");
    const [dismissedRecipes, setDismissedRecipes] = useState<Set<string>>(
      new Set(),
    );
      ...
      async (
        ...
        try {
          await trackInteraction({
            ...
            interactionValue:
              feedback === "like" ? 5 : feedback === "dislike" ? 1 : 0,
            metadata: {
    ...
    return (
      <div className={cn("w-full space-y-6", className)}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight flex items-center gap-2">
              <currentConfig.icon
                className={cn("h-6 w-6", currentConfig.color)}
              />
        ...
        <Tabs
          value={activeType}
          onValueChange={(value: any) => setActiveType(value)}
        >
          ...
          <TabsContent value={activeType} className="mt-6">
            ...
            {!isLoading && visibleRecommendations.length === 0 && (
              <Card>
                <CardContent className="flex flex-col items-center justify-center p-8">
                  <div className="text-center">
                    <currentConfig.icon
                      className={cn("h-12 w-12 text-gray-300 mx-auto mb-4")}
                    />
                    <h3 className="text-lg font-semibold mb-2">
            ...
            {!isLoading && visibleRecommendations.length > 0 && (
              <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {visibleRecommendations.map(
                  (item: RecommendedRecipe, index: number) => (
            ...
            {!isLoading &&
              ...
              visibleRecommendations.length >= 15 && (
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      ...
                      The more feedback you give, the better our suggestions
                      become!
                    </p>
                  </div>
                </div>
