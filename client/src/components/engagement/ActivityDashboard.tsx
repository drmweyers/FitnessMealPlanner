...
const ActivityDashboard = memo(({ className }: ActivityDashboardProps) => {
  ...
  const derivedMetrics = useMemo(() => {
    ...
    const engagementRate =
      stats.recipesViewed > 0
        ? (stats.recipesRated / stats.recipesViewed) * 100
        : 0;
    const favoriteRate =
      stats.recipesViewed > 0
        ? (stats.totalFavorites / stats.recipesViewed) * 100
        : 0;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Tabs
            ...
            onValueChange={(value) =>
              setSelectedPeriod(Number(value) as 7 | 30 | 90)
            }
          >
            ...
            {isLoading ? (
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  <Progress
                    value={derivedMetrics.favoriteRate}
                    className="h-2"
                  />
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="space-y-2">
                  ...
                  <Progress
                    value={Math.min(
                      (derivedMetrics.avgInteractionsPerDay / 10) * 100,
                      100,
                    )}
                    className="h-2"
                  />
                    ...
                    <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                      ...
                      <div className="space-y-1 text-sm text-blue-700">
                        {stats.totalFavorites < 10 && (
                          <p>
                            • Favorite {10 - stats.totalFavorites} more recipes
                            to unlock Recipe Collector
                          </p>
                        )}
                        {stats.recipesRated < 20 && (
                          <p>
                            • Rate {20 - stats.recipesRated} more recipes to
                            unlock Active Reviewer
                          </p>
                        )}
                        {stats.totalCollections < 3 && (
                          <p>
                            • Create {3 - stats.totalCollections} more
                            collections to unlock Organization Master
                          </p>
                        )}
                        {stats.recipesViewed < 50 && (
                          <p>
                            • View {50 - stats.recipesViewed} more recipes to
                            unlock Recipe Explorer
                          </p>
                        )}
                      </div>
                    </div>
