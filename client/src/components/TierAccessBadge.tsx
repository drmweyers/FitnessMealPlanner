...
export function TierAccessBadge() {
  ...
  const tierColors = {
    ...
    professional: "bg-orange-100 text-orange-800 hover:bg-orange-200",
    enterprise: "bg-amber-100 text-amber-800 hover:bg-amber-200",
  };

  const tierColor =
    tierColors[
      tierName.toLowerCase() as "starter" | "professional" | "enterprise"
    ] || tierColors.starter;
        ...
        <TooltipContent className="max-w-xs">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-sm">
              <ChefHat className="h-4 w-4" />
              <span>
                Access to {accessibleRecipeCount.toLocaleString()} recipes
              </span>
            </div>
