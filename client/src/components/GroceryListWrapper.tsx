...
const GroceryListWrapper: React.FC<GroceryListWrapperProps> = ({
  ...
  const filteredLists = useMemo(() => {
    ...
    const filtered = groceryLists.filter(
      (list) =>
        list &&
        list.id &&
        list.name &&
  ...
  useEffect(() => {
    ...
    if (
      !listsLoading &&
      Array.isArray(groceryLists) &&
      groceryLists.length > 0 &&
      !selectedListId
    ) {
      // Prioritize lists with items
      const listsWithItems = groceryLists.filter(
        (list) => list && list.id && list.name && (list.itemCount ?? 0) > 0,
      );
      const validLists = groceryLists.filter(
        (list) => list && list.id && list.name,
      );
      ...
      if (listsWithItems.length > 0) {
        ...
        console.log(
          "[GroceryListWrapper] Auto-selecting valid list:",
          listToSelect,
        );
  ...
  if (listsLoading) {
    return (
      <div
        className={`min-h-screen bg-background ${className} flex items-center justify-center`}
      >
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-md mx-auto">
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-r from-orange-500 to-orange-600 rounded-2xl mb-4 shadow-lg shadow-orange-500/25">
