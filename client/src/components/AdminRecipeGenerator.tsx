...
export default function AdminRecipeGenerator() {
  ...
  const connectToProgressStream = (jobId: string) => {
    ...
    const eventSource = new EventSource(
      `/api/admin/bmad-progress-stream/${jobId}`,
    );
    ...
    eventSource.addEventListener("progress", (event) => {
      try {
        ...
        setGenerationProgress(
          stepNames[progress.currentStep] || progress.currentStep,
        );
        ...
        if (
          ...
          if (progress.currentStep === "complete") {
            ...
            invalidateRecipeQueries(
              queryClient,
              "AdminRecipe-Generation-Complete",
            );
          } else {
    ...
    eventSource.onerror = (error) => {
      ...
      toast({
        ...
        description:
          "Lost connection to progress updates. Refresh to see final results.",
        variant: "destructive",
      });
    };
  };
  ...
  const generateRecipes = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (data.batchId) {
        console.log(
          "[BMAD Recipe Generation] Connecting to SSE with batchId:",
          data.batchId,
        );
        connectToProgressStream(data.batchId);
      } else {
        console.warn(
          "[BMAD Recipe Generation] No batchId received, cannot track progress",
        );
  ...
  const parseNaturalLanguage = useMutation({
    ...
    onSuccess: (data) => {
      ...
      console.log(
        `[Parse Button] ✅ Successfully populated ${fieldsPopulated} form fields`,
      );
    ...
    onError: (error: Error) => {
      ...
      toast({
        ...
        description:
          error.message || "Failed to parse prompt. Check console for details.",
        variant: "destructive",
      });
    },
  });
  ...
  const handleNaturalLanguageParse = () => {
    ...
    toast({
      ...
      description:
        "Analyzing your prompt with AI. This may take a few seconds.",
    });
  ...
  const handleDirectGeneration = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.message || "Failed to start natural language generation",
        );
      }
      ...
      if (result.jobId) {
        console.log(
          "[Natural Language Generation] Connecting to SSE with jobId:",
          result.jobId,
        );
      ...
      console.log(
        "[Natural Language Generation] Started recipe generation:",
        result,
      );
      console.log(
        "[Natural Language Generation] Parsed parameters:",
  ...
  const handleRefreshStats = () => {
    ...
    queryClient.refetchQueries({
      predicate: (query) => query.queryKey[0] === "/api/admin/stats",
    });
    toast({
  ...
  const handleRefreshPendingRecipes = () => {
    ...
    queryClient.refetchQueries({
      predicate: (query) => query.queryKey[0] === "/api/admin/recipes",
    });
    queryClient.refetchQueries({
      predicate: (query) => query.queryKey[0] === "/api/admin/stats",
    });
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          {!isCollapsed && (
            <CardDescription>
              Generate custom recipes using AI based on specific dietary
              requirements and nutritional targets.
            </CardDescription>
          )}
        </CardHeader>
        {!isCollapsed && (
            ...
            <Card className="mb-6 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="text-blue-600">
                  Describe your recipe generation requirements in plain English
                  and let AI automatically fill the form below.
                </CardDescription>
              </CardHeader>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Generate 1-50 recipes per batch
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Optional main ingredient to focus on
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
      ...
      {(lastGeneration || isGenerating) && (
        ...
        >
          <CardContent className="pt-6">
            <div
              className={`flex items-center gap-2 ${isGenerating ? "text-blue-700" : "text-green-700"}`}
            >
              {isGenerating ? (
            ...
            {lastGeneration && (
              <>
                <p
                  className={`${isGenerating ? "text-blue-600" : "text-green-600"} mt-4`}
                >
                  {lastGeneration.message}
                </p>
                <div className="flex items-center gap-3 mt-3">
                  <Badge variant="secondary">
                    {lastGeneration.count} recipes{" "}
