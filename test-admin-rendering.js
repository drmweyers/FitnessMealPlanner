...
async function testAdminRendering() {
  ...
  try {
    ...
    const adminPageResponse = await fetch('http://localhost:5000/admin');
    ...
    if (fs.existsSync(adminPath)) {
      ...
      const hasRecipeManagement = content.includes('Recipe Database Management');
    ...
    if (fs.existsSync(appPath)) {
      ...
      const hasAdminRoute = appContent.includes('/admin') || appContent.includes('Admin');
