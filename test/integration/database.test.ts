...
import { createTestUsers, setupTestDatabase, cleanupTestDatabase } from '../helpers/database-helpers';
...
describe('Database Operations', () => {
  ...
  describe('Recipe CRUD Operations', () => {
    ...
    it('should return undefined for non-existent recipe', async () => {
      const recipe = await storage.getRecipe(randomUUID());
      expect(recipe).toBeUndefined();
    });

    it('should return false when deleting non-existent recipe', async () => {
      const deleted = await storage.deleteRecipe(randomUUID());
  ...
  describe('User Operations', () => {
    ...
    it('should create/upsert a user', async () => {
      ...
      expect(user.name).toBe(mockUser.firstName + ' ' + mockUser.lastName);
    ...
    it('should return undefined for non-existent user', async () => {
      const user = await storage.getUser(randomUUID());
  ...
  describe('Trainer Meal Plan Operations', () => {
    ...
    const mockTrainer = {
      email: `trainer-${randomUUID()}@test.com`,
      name: 'Test Trainer',
      role: 'trainer',
    ...
    const mockCustomer = {
      ...
      name: 'Test Customer',
      role: 'customer',
      profilePicture: 'https://example.com/customer.jpg'
    };
