...
const recipeSchema = z.object({
  ...
  proteinGrams: z
    .string()
    .refine((val) => !isNaN(parseFloat(val)), "Invalid protein value"),
  carbsGrams: z
    .string()
    .refine((val) => !isNaN(parseFloat(val)), "Invalid carbs value"),
  fatGrams: z
    .string()
    .refine((val) => !isNaN(parseFloat(val)), "Invalid fat value"),
...
function transformMealPlanData(data: any): any {
  ...
  if (data.name && !data.planName) {
    mealPlanData = {
      ...
      dailyCalorieTarget:
        data.dailyCalorieTarget || calculateDailyCalories(data.meals),
...
function transformMealsFromObject(mealsObj: any): any[] {
  ...
  Object.entries(mealsObj).forEach(([dayName, dayMeals]: [string, any]) => {
    ...
    Object.entries(dayMeals).forEach(
      ([mealType, mealData]: [string, any], mealIndex) => {
...
function calculateDailyCalories(meals: any[]): number {
  ...
  const totalCalories = meals.reduce((sum, meal) => {
    const calories =
      meal.recipe?.caloriesKcal || meal.caloriesKcal || meal.calories || 400;
    return sum + calories;
  }, 0);
 ...
 */
export async function validateMealPlanData(
  data: any,
  ...
  try {
    ...
    // Always use the internal PDF schema — the shared schema is for DB storage
    // and has different required fields (generatedBy, createdAt) and optional
    // handling that can strip ingredientsJson needed for shopping lists.
    const validated = mealPlanSchema.parse(dataWithDefaults);
...
function validateMealPlanLogic(mealPlan: MealPlanPdfData): void {
  ...
  if (invalidDays.length > 0) {
    throw new Error(
      `Meals found for days beyond plan duration: ${invalidDays.map((m) => m.day).join(", ")}`,
    );
  }
  ...
  Object.entries(mealsByDay).forEach(([day, meals]) => {
    const dayCalories = meals.reduce(
      (sum, meal) => sum + meal.recipe.caloriesKcal,
      0,
    );
  ...
  const invalidMealTypes = mealPlan.meals.filter(
    (meal) => !validMealTypes.includes(meal.mealType.toLowerCase()),
  );

  if (invalidMealTypes.length > 0) {
    console.warn(
      `Unusual meal types found: ${invalidMealTypes.map((m) => m.mealType).join(", ")}`,
    );
  }
}
...
function formatFraction(decimal: number): string {
  ...
  const closestFraction = fractions.reduce((prev, curr) =>
    Math.abs(curr.decimal - fractional) < Math.abs(prev.decimal - fractional)
      ? curr
      : prev,
  );
  ...
  if (Math.abs(closestFraction.decimal - fractional) < 0.05) {
    return whole > 0
      ? `${whole} ${closestFraction.fraction}`
