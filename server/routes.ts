...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const filters = recipeFilterSchema.parse({
        ...
        minProtein: req.query.minProtein
          ? parseInt(req.query.minProtein as string)
          : undefined,
        maxProtein: req.query.maxProtein
          ? parseInt(req.query.maxProtein as string)
          : undefined,
        minCarbs: req.query.minCarbs
          ? parseInt(req.query.minCarbs as string)
          : undefined,
        maxCarbs: req.query.maxCarbs
          ? parseInt(req.query.maxCarbs as string)
          : undefined,
        minFat: req.query.minFat
          ? parseInt(req.query.minFat as string)
          : undefined,
        maxFat: req.query.maxFat
          ? parseInt(req.query.maxFat as string)
          : undefined,
    ...
    try {
      ...
      if (!planName || !fitnessGoal || !dailyCalorieTarget) {
        return res.status(400).json({
          message:
            "Missing required fields: planName, fitnessGoal, dailyCalorieTarget",
        });
      }
      ...
      if ((error as Error).message.includes("No approved recipes")) {
        errorMessage =
          "No recipes available in the database. Please add some recipes first.";
  ...
  app.post("/api/meal-plan/parse-natural-language", async (req, res) => {
    try {
      ...
      const { parseNaturalLanguageMealPlan } =
        await import("./services/openai");
      const parsedPlan =
        await parseNaturalLanguageMealPlan(naturalLanguageInput);
  ...
  app.post(
    "/api/admin/parse-recipe-requirements",
    requireAuth,
    requireRole("admin"),
    async (req, res) => {
      try {
        ...
        if (!naturalLanguageInput || typeof naturalLanguageInput !== "string") {
          return res
            .status(400)
            .json({ message: "Natural language input is required" });
        }

        const { parseNaturalLanguageRecipeRequirements } =
          await import("./services/openai");
        const parsedRequirements =
          await parseNaturalLanguageRecipeRequirements(naturalLanguageInput);

        res.json(parsedRequirements);
      } catch (error) {
        console.error(
          "Error parsing natural language recipe requirements:",
          error,
        );
        res.status(500).json({
          message: "Failed to parse natural language input",
    ...
    async (req, res) => {
      try {
        ...
        if (typeof count !== "number" || count < 1 || count > 100) {
          return res
            .status(400)
            .json({ message: "Count must be a number between 1 and 100" });
        }
