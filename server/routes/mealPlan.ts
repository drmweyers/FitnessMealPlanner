...
// Fields on the Advanced Macro/Nutrition Filter — tier-gated to Pro+
...
mealPlanRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const parsedData =
        await parseNaturalLanguageForMealPlan(naturalLanguageInput);
...
mealPlanRouter.post("/generate", requireAuth, async (req, res) => {
  ...
  // Tier gate — Advanced Macro/Nutrition Filter is Pro+ only.
  // Silent strip: drop advanced fields from Starter requests so the endpoint
  // keeps working (basic plans still generate). The client wraps the Advanced
  // Filter UI in an upgrade overlay so most Starter trainers never reach here
  // with advanced fields. This also protects against a stale client.
  //
  // Fail-safe on entitlements error: if the entitlements lookup throws
  // (Redis outage, DB timeout), we DEFAULT TO STARTER and strip the advanced
  // fields. Basic plan generation still succeeds. This is deliberate — better
  // for a Pro user to briefly lose advanced filters than for a Starter to
  // silently get the paid feature during an infra blip.
  let resolvedTier: "starter" | "professional" | "enterprise" = "starter";
  try {
    const entitlements = await entitlementsService.getEntitlements(userId);
    resolvedTier = (entitlements?.tier as any) ?? "starter";
  } catch (err) {
    console.warn(
      "[Tier Gate] entitlements lookup failed; defaulting to starter (fail-safe):",
      err,
    );
  }
  if (resolvedTier === "starter") {
    const strippedFields: string[] = [];
    for (const field of ADVANCED_FILTER_FIELDS) {
      if (
        (mealPlanParams as any)[field] !== undefined &&
        (mealPlanParams as any)[field] !== null
      ) {
        strippedFields.push(field);
        delete (mealPlanParams as any)[field];
      }
    }
    if (strippedFields.length > 0) {
      console.log(
        `[Tier Gate] Starter trainer ${userId} — stripped advanced filter fields: ${strippedFields.join(", ")}`,
      );
    }
  }
...
mealPlanRouter.post("/generate-intelligent", requireAuth, async (req, res) => {
  ...
  try {
    console.log(
      "[Intelligent API] Generating intelligent meal plan with params:",
      mealPlanParams,
    );
    ...
    const nutrition =
      intelligentMealPlanGenerator.calculateMealPlanNutrition(mealPlan);

    // Get customer preferences for additional context
    const customerPrefs =
      await customerPreferenceService.getCustomerPreferences(
        mealPlanParams.clientName || "",
      );
    const preferenceAnalysis = customerPrefs
      ? customerPreferenceService.generatePreferenceAnalysis(customerPrefs)
      : null;
    ...
    console.log(
      "[Intelligent API] Intelligent meal plan generated successfully.",
    );
    res.json({
      mealPlan,
      nutrition,
      ...
      intelligentFeatures: {
        ...
        enhancedMealPrep:
          !!mealPlan.startOfWeekMealPrep?.intelligentBatchingTips,
      },
      message:
        "Intelligent meal plan generated successfully with AI optimization",
      completed: true,
      timestamp: new Date().toISOString(),
      usageInfo: (req as any).usageInfo, // Include usage info from middleware
    });
    ...
    console.error(
      "[Intelligent API] Error generating intelligent meal plan:",
      error,
    );
    const errorMessage =
      error instanceof Error ? error.message : "An unknown error occurred";
    res
      .status(500)
      .json({
        error: "Failed to generate intelligent meal plan",
        details: errorMessage,
      });
  }
});
...
mealPlanRouter.post("/optimize-nutrition", requireAuth, async (req, res) => {
  ...
  try {
    ...
    const defaultConstraints = {
      ...
      fatMax: (mealPlan.dailyCalorieTarget * 0.4) / 9, // 40% of calories from fat
      ...constraints, // Override with provided constraints
    };

    const optimizationResult =
      await nutritionalOptimizer.optimizeMealPlanNutrition(
        mealPlan,
        defaultConstraints,
        customerPreferences,
      );
    ...
    const optimizationReport =
      nutritionalOptimizer.generateOptimizationReport(optimizationResult);

    console.log(
      "[Nutrition Optimizer API] Optimization completed successfully.",
    );
    res.json({
      optimizationResult,
      optimizationReport,
    ...
    console.error(
      "[Nutrition Optimizer API] Error optimizing nutrition:",
      error,
    );
    const errorMessage =
      error instanceof Error ? error.message : "An unknown error occurred";
    res
      .status(500)
      .json({ error: "Failed to optimize nutrition", details: errorMessage });
  }
});
...
mealPlanRouter.get(
  ...
  async (req, res) => {
    ...
    try {
      console.log(
        "[Customer Preferences API] Fetching preferences for customer:",
        customerId,
      );

      const preferences =
        await customerPreferenceService.getCustomerPreferences(customerId);
      ...
      const analysis =
        customerPreferenceService.generatePreferenceAnalysis(preferences);
      ...
      console.error(
        "[Customer Preferences API] Error fetching preferences:",
        error,
      );
      const errorMessage =
        error instanceof Error ? error.message : "An unknown error occurred";
...
mealPlanRouter.post("/create-schedule", requireAuth, async (req, res) => {
  ...
  if (!mealPlan || !customerId) {
    return res
      .status(400)
      .json({ error: "Meal plan and customer ID are required" });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to create meal schedule", details: errorMessage });
  }
});
...
mealPlanRouter.post("/create-variation", requireAuth, async (req, res) => {
  const { mealPlan, customerId, variationType, variationParameters } = req.body;
  const userId = req.user?.id;
  ...
  try {
    console.log(
      "[Meal Plan Variation API] Creating variation:",
      variationType || "intelligent",
    );
    ...
    res
      .status(500)
      .json({
        error: "Failed to create meal plan variation",
        details: errorMessage,
      });
  }
});
...
mealPlanRouter.post("/create-rotation-plan", requireAuth, async (req, res) => {
  ...
  if (!customerId || !baseMealPlan) {
    return res
      .status(400)
      .json({ error: "Customer ID and base meal plan are required" });
  }

  try {
    console.log(
      "[Rotation Plan API] Creating rotation plan for customer:",
      customerId,
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to create rotation plan", details: errorMessage });
  }
});

// GET /api/meal-plan/progressive/:customerId/:weekNumber - Generate progressive meal plan
mealPlanRouter.get(
  "/progressive/:customerId/:weekNumber",
  requireAuth,
  async (req, res) => {
    ...
    try {
      console.log(
        "[Progressive API] Generating progressive meal plan for week:",
        weekNumber,
      );
      ...
      if (!baseMealPlan) {
        return res
          .status(400)
          .json({ error: "Base meal plan parameters required" });
      }

      const baseParams = JSON.parse(baseMealPlan as string);

      const progressiveMealPlan =
        await intelligentMealPlanGenerator.generateProgressiveMealPlan(
          baseParams,
          userId,
          parseInt(weekNumber),
      ...
      const nutrition =
        intelligentMealPlanGenerator.calculateMealPlanNutrition(
          progressiveMealPlan,
        );
      ...
      console.error(
        "[Progressive API] Error generating progressive meal plan:",
        error,
      );
      const errorMessage =
        error instanceof Error ? error.message : "An unknown error occurred";
      res
        .status(500)
        .json({
          error: "Failed to generate progressive meal plan",
          details: errorMessage,
        });
    }
  },
...
mealPlanRouter.get("/personalized", requireAuth, async (req, res) => {
  try {
    ...
    const { mealPlanAssignments, trainerMealPlans, users } =
    ...
    const assignedPlans = await db
      ...
      .innerJoin(
        trainerMealPlans,
        eq(trainerMealPlans.id, mealPlanAssignments.mealPlanId),
      )
    ...
    res.json({
      ...
      summary: {
        ...
        totalCalorieTargets: enhancedMealPlans.reduce(
          (sum, p) => sum + (p.dailyCalorieTarget || 0),
          0,
        ),
        avgCaloriesPerDay:
          enhancedMealPlans.length > 0
            ? Math.round(
                enhancedMealPlans.reduce(
                  (sum, p) => sum + (p.dailyCalorieTarget || 0),
                  0,
                ) / enhancedMealPlans.length,
              )
            : 0,
      },
    });
...
mealPlanRouter.delete("/:id", requireAuth, async (req, res) => {
  ...
  if (userRole !== "customer") {
    return res
      .status(403)
      .json({ error: "Only customers can delete their meal plan assignments" });
  }

  try {
    // Import required tables
    const { mealPlanAssignments, trainerMealPlans } =
      await import("@shared/schema");
    ...
    if (!assignment || assignment.length === 0) {
      return res.status(404).json({
        error:
          "Meal plan assignment not found or you do not have permission to delete it",
      });
    }
    ...
    try {
      ...
      if (cleanupResult.success && cleanupResult.action === "updated") {
        console.log(
          `[Meal Plan Delete] Cleaned up ${cleanupResult.itemCount || 0} orphaned grocery lists for customer ${userId} meal plan ${mealPlanId}`,
        );
      }
    } catch (error) {
      console.error(
        `[Meal Plan Delete] Error during grocery list cleanup for meal plan ${mealPlanId}:`,
        error,
      );
    ...
    console.log(
      `[Meal Plan Delete] Customer ${userId} deleted assignment for meal plan ${mealPlanId}`,
    );
