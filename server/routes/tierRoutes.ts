// @ts-nocheck - Role array types need middleware refactor
/**
    ...
    const pricing = {
      ...
      ai: {
        ...
        enterprise: {
          name: "AI Enterprise",
          stripePriceId:
            process.env.STRIPE_PRICE_AI_ENTERPRISE ||
            "price_ai_enterprise_monthly",
          amount: 19900, // $199.00 in cents
          currency: "usd",
          interval: "month",
...
tierRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { tier, successUrl, cancelUrl, trialPeriodDays } =
      ...
      if (
        ...
        error.message?.includes("cus_test_")
      ) {
        return res.status(400).json({
          error: "Test Account Detected",
          message:
            "This is a test account. Payment features are not available for test accounts. Please use a production account to process payments.",
          isTestAccount: true,
        });
      }
...
tierRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const session =
        await stripeSubscriptionService.createBillingPortalSession(
          trainerId,
          returnUrl,
        );
...
tierRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const subscription =
        await stripeSubscriptionService.cancelAiSubscription(trainerId);
...
tierRouter.post("/webhooks/stripe", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.success) {
      ...
      res
        .status(500)
        .json({ error: "Webhook processing failed", details: result.error });
    }
  } catch (error: any) {
