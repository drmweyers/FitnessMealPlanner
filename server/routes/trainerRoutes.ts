...
import { z } from "zod";
import {
  manualMealPlanService,
  type ManualMealEntry,
} from "../services/manualMealPlanService";
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const [clientsWithMealPlans] = await db
        .select({
      ...
      const [clientsWithRecipes] = await db
        .select({
      ...
      const [mealPlansCreated] = await db
        .select({
      ...
      const [recipesAssigned] = await db
        .select({
          count: sql<number>`count(*)::int`,
        })
        .from(personalizedRecipes)
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const { customerInvitations, mealPlanAssignments } =
        await import("@shared/schema");

      // Get customers who have accepted invitations from this trainer
      const invitedCustomers = await db
        .select({
          customerId: users.id,
      ...
      const customersFromAssignments = await db
        .select({
      ...
      invitedCustomers.forEach((customer) => {
        customerMap.set(customer.customerId, {
          ...
          hasRecipe: recipeId
            ? customersWithThisRecipe.has(customer.customerId)
            : false,
      ...
      customersFromAssignments.forEach((customer) => {
        if (!customerMap.has(customer.customerId)) {
          ...
          if (
            ...
            customer.assignedAt < existing.firstAssignedAt
          ) {
      ...
      [...customersWithMealPlans, ...customersWithRecipes].forEach(
        (customer) => {
          if (!customerMap.has(customer.customerId)) {
            customerMap.set(customer.customerId, {
              id: customer.customerId,
              ...
              hasRecipe: recipeId
                ? customersWithThisRecipe.has(customer.customerId)
                : false,
              hasMealPlan: customersWithMealPlans.some(
...
trainerRouter.get(
  "/customers/:customerId/meal-plans",
  requireAuth,
  requireRole("trainer"),
  async (req, res) => {
    try {
...
trainerRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        "[Bulk Assign] Request from:",
        req.user?.email,
        "Role:",
        req.user?.role,
      );
      console.log(
        "[Bulk Assign] Assigning to",
        customerIds.length,
        "customers",
      );
      ...
      const customers = await db
        .select()
        .from(users)
        .where(and(inArray(users.id, customerIds), eq(users.role, "customer")));
      ...
      console.log(
        "[Bulk Assign] SUCCESS: Assigned to",
        assignments.length,
        "customers",
      );
...
// mealPlanData must be a non-null object (any shape — the planner schema is
// large and validated client-side). Without this guard, an empty body parsed
// as `{ mealPlanData: undefined }` and crashed db.insert with a NOT NULL
// constraint, returning 500 instead of 400.
const saveMealPlanSchema = z.object({
  mealPlanData: z.record(z.any()),
...
trainerRouter.post(
  ...
  async (req, res) => {
    try {
      const trainerId = req.user!.id;
      const { mealPlanData, notes, tags, isTemplate } =
        saveMealPlanSchema.parse(req.body);

      console.log(
        "[Save Meal Plan] Request from user:",
        req.user?.email,
        "Role:",
        req.user?.role,
      );
      console.log(
        "[Save Meal Plan] Plan name:",
        (mealPlanData as any)?.planName || "Unknown",
      );
      ...
      console.log(
        "[Save Meal Plan] SUCCESS: Meal plan saved with ID:",
        savedPlan.id,
      );
...
trainerRouter.delete(
  "/meal-plans/:planId/assign/:customerId",
  requireAuth,
  requireRole("trainer"),
  async (req, res) => {
    try {
      ...
      const unassigned = await storage.unassignMealPlanFromCustomer(
        planId,
        customerId,
      );
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const relationships =
        await customerRelationshipManager.getCustomerRelationships(
          trainerId,
          filters,
        );
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      const trainerId = req.user!.id;
      const stats =
        await customerRelationshipManager.getTrainerDashboardStats(trainerId);
...
trainerRouter.get(
  "/customers/:customerId/progress-timeline",
  requireAuth,
  requireRole("trainer"),
  async (req, res) => {
    try {
      ...
      const timeline =
        await customerRelationshipManager.getCustomerProgressTimeline(
          trainerId,
          customerId,
          days,
        );
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const history =
        await assignmentHistoryTracker.getAssignmentHistory(filters);
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const statistics = await assignmentHistoryTracker.getAssignmentStatistics(
        trainerId,
        days,
      );
...
trainerRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const trends = await assignmentHistoryTracker.getAssignmentTrends(
        trainerId,
        period,
        duration,
      );
  ...
  async (req, res) => {
    try {
      ...
      if (format === "csv") {
        ...
        res.setHeader(
          "Content-Disposition",
          'attachment; filename="assignment-history.csv"',
        );
        res.send(exported);
      } else {
        res.json({
...
trainerRouter.get(
  "/customers/:customerId/assignment-history",
  requireAuth,
  requireRole("trainer"),
  async (req, res) => {
    try {
      ...
      const history =
        await assignmentHistoryTracker.getCustomerAssignmentHistory(
          trainerId,
          customerId,
        );
...
trainerRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      await customerRelationshipManager.updateCustomerRelationshipStatus(
        trainerId,
        customerId,
        status,
        reason,
...
const manualMealPlanSchema = z.object({
  ...
  meals: z
    ...
    .max(50, "Maximum 50 meals per plan"),
  notes: z.string().optional(),
  tags: z.array(z.string()).optional(),
...
trainerRouter.post(
  ...
  async (req, res) => {
    try {
      const trainerId = req.user!.id;
      const { planName, meals, notes, tags, isTemplate } =
        manualMealPlanSchema.parse(req.body);
 ...
 */
function convertImportToMealPlan(
  template: ImportMealPlan,
  trainerId: string,
): any {
  // Group meals by day
  ...
  async (req, res) => {
    try {
      ...
      const savedPlan = await storage.createTrainerMealPlan({
        ...
        tags: [
          template.dietaryProtocol || "balanced",
          "imported",
          "template",
          "tripwire",
        ],
      ...
      const { templates } = z
        .object({
          templates: z
            ...
            .max(100, "Maximum 100 templates per bulk import"),
        })
        .parse(req.body);
