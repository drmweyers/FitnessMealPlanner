...
import {
  attachRecipeTierFilter,
  getUserTierLevel,
} from "../middleware/tierEnforcement";
import { db } from "../db";
import {
  recipeRatings,
  recipeRatingSummary,
...
recipeRouter.get(
  ...
  async (req, res) => {
    try {
      ...
      const recipeTierLevel =
        tierLevels[recipe.tierLevel as keyof typeof tierLevels] || 1;
  ...
  async (req, res) => {
    try {
      ...
      const filters = {
        ...
        mealTypes: req.query.mealTypes
          ? (req.query.mealTypes as string).split(",")
          : undefined,
        dietaryTags: req.query.dietaryTags
          ? (req.query.dietaryTags as string).split(",")
          : undefined,
        calories:
          req.query.caloriesMin || req.query.caloriesMax
            ? {
                min: req.query.caloriesMin
                  ? Number(req.query.caloriesMin)
                  : undefined,
                max: req.query.caloriesMax
                  ? Number(req.query.caloriesMax)
                  : undefined,
              }
            : undefined,
        protein:
          req.query.proteinMin || req.query.proteinMax
            ? {
                min: req.query.proteinMin
                  ? Number(req.query.proteinMin)
                  : undefined,
                max: req.query.proteinMax
                  ? Number(req.query.proteinMax)
                  : undefined,
              }
            : undefined,
        carbs:
          req.query.carbsMin || req.query.carbsMax
            ? {
                min: req.query.carbsMin
                  ? Number(req.query.carbsMin)
                  : undefined,
                max: req.query.carbsMax
                  ? Number(req.query.carbsMax)
                  : undefined,
              }
            : undefined,
        fat:
          req.query.fatMin || req.query.fatMax
            ? {
            ...
            : undefined,
        prepTime:
          req.query.prepTimeMin || req.query.prepTimeMax
            ? {
                min: req.query.prepTimeMin
                  ? Number(req.query.prepTimeMin)
                  : undefined,
                max: req.query.prepTimeMax
                  ? Number(req.query.prepTimeMax)
                  : undefined,
              }
            : undefined,
        cookTime:
          req.query.cookTimeMin || req.query.cookTimeMax
            ? {
                min: req.query.cookTimeMin
                  ? Number(req.query.cookTimeMin)
                  : undefined,
                max: req.query.cookTimeMax
                  ? Number(req.query.cookTimeMax)
                  : undefined,
              }
            : undefined,
      ...
      console.log(
        `[Recipe Search API] Returning ${results.recipes.length} recipes out of ${results.total} total (tier: ${userTier})`,
      );
...
recipeRouter.get("/search/statistics", requireAuth, async (req, res) => {
  try {
    ...
    console.error(
      "[Recipe Search API] Failed to get search statistics:",
      error,
    );
    res.status(500).json({
      success: false,
