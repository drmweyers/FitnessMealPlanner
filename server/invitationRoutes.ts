  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const existingInvitations =
        await storage.getInvitationsByTrainer(trainerId);
      ...
      if (emailResult.success) {
        console.log(
          `Invitation email sent successfully to ${customerEmail}, messageId: ${emailResult.messageId}`,
        );
      } else {
        console.warn(
          `Failed to send invitation email to ${customerEmail}: ${emailResult.error}`,
        );
      }
