  ...
  try {
    ...
    if (!token) {
      ...
      try {
        ...
        const newTokens = await refreshTokensWithDeduplication(
          user.id,
          refreshToken,
        );
...
/**
 * Optional auth: best-effort token decode for routes that are public but
 * want to know who the caller is if a valid token is present. Unlike
 * requireAuth, this middleware never returns 401 — it just leaves req.user
 * unset on any failure and calls next(). Use for public routes that apply
 * tier-based filtering (e.g. GET /api/recipes).
 */
export const optionalAuth = async (
  ...
  try {
    ...
    // swallow — optional auth never blocks
  }
  next();
};
...
export const requireTrainerOrAdmin = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
