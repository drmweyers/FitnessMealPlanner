...
export class MealPlanGeneratorService {
  ...
  async generateMealPlan(
    ...
    if (filterParams.dietaryTag) {
      ...
      const normalizedTag = filterParams.dietaryTag
        .toLowerCase()
        .replace(/_/g, "-");
      recipeFilter.dietaryTag = normalizedTag;
      console.log(
        `[Meal Plan Generator] Filtering by dietary tag: ${normalizedTag}`,
      );
    }
    if (filterParams.maxPrepTime)
      recipeFilter.maxPrepTime = filterParams.maxPrepTime;
    // Apply nutrition range filters if provided
    if (filterParams.maxCalories)
      recipeFilter.maxCalories = filterParams.maxCalories;
    if (filterParams.minCalories)
      recipeFilter.minCalories = filterParams.minCalories;
    if (filterParams.minProtein)
      recipeFilter.minProtein = filterParams.minProtein;
    if (filterParams.maxProtein)
      recipeFilter.maxProtein = filterParams.maxProtein;
    ...
    if (description) {
      ...
      console.log(
        `[Meal Plan Generator] Extracted description keywords: ${descriptionKeywords.join(", ")}`,
      );
    }
    ...
    if (descriptionKeywords.length > 0) {
      const keywordFiltered = recipes.filter((recipe) => {
        ...
        const mainIngredients = (recipe.mainIngredientTags || []).map(
          (tag: string) => tag.toLowerCase(),
        );
        const hasInMainIngredients = descriptionKeywords.some((keyword) =>
          mainIngredients.some(
            (tag) => tag.includes(keyword) || tag === keyword,
          ),
        );

        // SECONDARY: Check ingredient names
        const allIngredientNames = (recipe.ingredientsJson || []).map(
          (ing: any) => (ing.name || "").toLowerCase(),
        );
        const hasInIngredients = descriptionKeywords.some((keyword) =>
          allIngredientNames.some(
            (name) => name.includes(keyword) || name === keyword,
          ),
        );
        ...
        const hasInNameOrDesc = descriptionKeywords.some(
          (keyword) =>
      ...
      if (keywordFiltered.length > 0) {
        console.log(
          `[Meal Plan Generator] ✅ Filtered to ${keywordFiltered.length} recipes matching description keywords: ${descriptionKeywords.join(", ")}`,
        );
        ...
        const availableIngredients = Array.from(
          new Set(
            recipes.flatMap((r) =>
              (r.mainIngredientTags || []).map((tag: string) =>
                tag.toLowerCase(),
              ),
            ),
          ),
        ...
        console.error(
          `[Meal Plan Generator] ❌ ERROR: No recipes found matching description keywords: ${descriptionKeywords.join(", ")}.`,
        );
        console.error(
          `[Meal Plan Generator] Available main ingredients: ${availableIngredients.join(", ") || "None found"}`,
        );
        console.error(
          `[Meal Plan Generator] Available ingredient names: ${availableIngredientNames.slice(0, 20).join(", ") || "None found"}${availableIngredientNames.length > 20 ? "..." : ""}`,
        );
        const validationError: any = new Error(
          `No recipes found matching your description requirement: "${descriptionKeywords.join(", ")}". Available main ingredients: ${availableIngredients.join(", ") || "None"}. Please adjust your description or add recipes with these ingredients to your library.`,
        );
    ...
    if (recipes.length === 0) {
      ...
      allRecipesResult.recipes.forEach((recipe) => {
        (recipe.dietaryTags || []).forEach((tag) =>
          availableTags.add(tag.toLowerCase()),
        );
      });
      ...
      const filterSummary =
        appliedFilters.length > 0 ? ` with ${appliedFilters.join(", ")}` : "";

      const normalizedRequestedTag = filterParams.dietaryTag
        ? filterParams.dietaryTag.toLowerCase().replace(/_/g, "-")
        : null;
      console.error(
        `[Meal Plan Generator] ❌ ERROR: No recipes found${filterSummary}.`,
      );
      if (normalizedRequestedTag) {
        console.error(
          `[Meal Plan Generator] Available dietary tags in database: ${Array.from(availableTags).join(", ") || "None found"}`,
        );
      }
      ...
      if (
        ...
        !availableTags.has(normalizedRequestedTag)
      ) {
    ...
    if (recipes.length === 0) {
      ...
      throw new Error(
        "No approved recipes available in the database. Please add some recipes first or check your database connection.",
      );
    }

    console.log(
      `Found ${recipes.length} approved recipes for meal plan generation`,
    );
    ...
    const mealPlan: MealPlan = {
      ...
      fitnessGoal,
      description:
        description || `${planName} - ${fitnessGoal} focused meal plan`,
      dailyCalorieTarget,
      clientName,
      days,
      mealsPerDay,
      ...
      for (let mealNumber = 1; mealNumber <= mealsPerDay; mealNumber++) {
        ...
        if (!params.mealType) {
          ...
          if (availableRecipes.length === 0) {
            console.warn(
              `[Meal Plan Generator] ⚠️  No recipes found for meal type '${mealType}' with current filters. Using all available filtered recipes.`,
            );
        ...
        if (filterParams.dietaryTag && availableRecipes.length > 0) {
          ...
          if (dietaryFiltered.length === 0) {
            console.error(
              `[Meal Plan Generator] ❌ ERROR: No recipes found for meal type '${mealType}' with dietary tag '${filterParams.dietaryTag}'.`,
            );
            const validationError: any = new Error(
              `No recipes found for ${mealType} matching your dietary requirement: "${filterParams.dietaryTag}". Please adjust your meal plan parameters or add more recipes to your library.`,
            );
            validationError.isValidationError = true;
            validationError.statusCode = 400;
            throw validationError;
        ...
        if (maxIngredients && maxIngredients > 0) {
          ...
          if (availableRecipes.length === 0) {
            console.warn(
              `[Meal Plan Generator] ⚠️  No recipes found with ${maxIngredients} or fewer ingredients for ${mealType}. Relaxing constraint to ${maxIngredients + 1}...`,
            );
            ...
            if (availableRecipes.length === 0) {
              console.warn(
                `[Meal Plan Generator] ⚠️  Still no recipes found. Using all available recipes for this meal.`,
              );
        ...
        const baseVariance =
          filterParams.minCalories || filterParams.maxCalories
            ? calorieVariance
            : Math.round(caloriesPerMeal * 0.5);
        const minCalories = filterParams.minCalories
          ? Math.max(
              filterParams.minCalories,
        ...
        let calorieFilteredRecipes = availableRecipes.filter(
          (recipe) =>
            recipe.caloriesKcal >= minCalories &&
            recipe.caloriesKcal <= maxCalories,
        );

        // Apply macro nutrient filters if specified
        if (
          filterParams.minProtein ||
          filterParams.maxProtein ||
          filterParams.minCarbs ||
          filterParams.maxCarbs ||
          filterParams.minFat ||
          filterParams.maxFat
        ) {
          calorieFilteredRecipes = calorieFilteredRecipes.filter((recipe) => {
            ...
            if (filterParams.minProtein && protein < filterParams.minProtein)
              return false;
            if (filterParams.maxProtein && protein > filterParams.maxProtein)
              return false;

            // Check carbs constraints
            if (filterParams.minCarbs && carbs < filterParams.minCarbs)
              return false;
            if (filterParams.maxCarbs && carbs > filterParams.maxCarbs)
              return false;
        ...
        if (calorieFilteredRecipes.length === 0) {
          console.log(
            `[Meal Plan Generator] No recipes match strict nutrition constraints (${minCalories}-${maxCalories} cal) for day ${day}, meal ${mealNumber}. Trying relaxed constraints...`,
          );
          ...
          calorieFilteredRecipes = availableRecipes.filter(
            (recipe) =>
              recipe.caloriesKcal >= relaxedMin &&
              recipe.caloriesKcal <= relaxedMax,
          );

          if (calorieFilteredRecipes.length === 0) {
            console.log(
              `[Meal Plan Generator] No recipes match relaxed constraints either. Using all available recipes.`,
            );
            calorieFilteredRecipes = availableRecipes;
          } else {
            console.log(
              `[Meal Plan Generator] Found ${calorieFilteredRecipes.length} recipes with relaxed constraints (${relaxedMin}-${relaxedMax} cal).`,
            );
          }
        }

        // Phase B+C (2026-04-13): plan-level dedup + variety enforcement.
        //
        // Before this refactor, the generator could return the same recipe 9
        // times in a 3-day × 3-meal plan because the "recent duplicates" check
        // only looked at the last `mealsPerDay * 2` meals and happily reused
        // earlier recipes. Worse, when the filtered pool was small the code
        // fell through to a random pick with zero dedup.
        //
        // New rules:
        //  B. Track usedRecipeIds across the ENTIRE plan. Prefer recipes not
        //     yet used. Only reuse when the unused pool is exhausted, and then
        //     prefer the least-recently-used (furthest back in mealPlan.meals).
        //  C. Track mainIngredientTag counts across the plan. A single main
        //     ingredient (e.g. "Beef") should not exceed ~40% of meals. Apply
        //     a variety score penalty to candidates whose main tag is already
        //     dominant.
        const totalMealsInPlan = days * mealsPerDay;
        const mainTagCap = Math.max(2, Math.ceil(totalMealsInPlan * 0.4));
        const usedRecipeIds = new Set<string>(
          mealPlan.meals.map((m: any) => m.recipe.id),
        );
        const mainTagCounts = new Map<string, number>();
        for (const m of mealPlan.meals) {
          for (const t of (m.recipe.mainIngredientTags || []) as string[]) {
            const key = String(t).toLowerCase();
            mainTagCounts.set(key, (mainTagCounts.get(key) || 0) + 1);
          }
        }

        const scoreVariety = (recipe: any): number => {
          const tags: string[] = (recipe.mainIngredientTags || []).map(
            (t: string) => String(t).toLowerCase(),
          );
          if (tags.length === 0) return 0;
          // Penalty proportional to how over-represented the recipe's tags are
          let penalty = 0;
          for (const tag of tags) {
            const count = mainTagCounts.get(tag) || 0;
            if (count >= mainTagCap)
              penalty += 100; // hard penalty: over cap
            else if (count > 0) penalty += count * 10; // soft penalty
          }
          return -penalty;
        };

        const pickFromPool = (pool: any[]): any => {
          if (pool.length === 0) return null;
          if (pool.length === 1) return pool[0];
          // Rank by variety score desc, then take top 30% window and random-pick
          const scored = pool
            .map((r) => ({ r, s: scoreVariety(r) }))
            .sort((a, b) => b.s - a.s);
        ...
        // Unused-first pool
        const unusedPool = calorieFilteredRecipes.filter(
          (r: any) => !usedRecipeIds.has(r.id),
        );

        let selectedRecipe: any;
        if (maxIngredients && maxIngredients > 0) {
          // Ingredient-limit path: use the dedicated scorer but against the
          // unused-first pool. Fall back to the full filtered pool only if
          // everything has been used.
          const primary =
            unusedPool.length > 0 ? unusedPool : calorieFilteredRecipes;
          ...
          selectedRecipe = pickFromPool(unusedPool);
        } else {
          // Hard fallback: every recipe in the filtered pool has been used
          // at least once. Allow reuse but prefer least-recently-used (the
          // one that appeared earliest in mealPlan.meals, which is naturally
          // the front of any stable order).
          const lastUsedIndex = new Map<string, number>();
          mealPlan.meals.forEach((m: any, i: number) =>
            lastUsedIndex.set(m.recipe.id, i),
          );
          const byLeastRecent = [...calorieFilteredRecipes].sort((a, b) => {
            const ai = lastUsedIndex.get(a.id) ?? -1;
            const bi = lastUsedIndex.get(b.id) ?? -1;
            return ai - bi;
          });
          // Take the 30% that were used longest ago and apply variety scoring
          const window = byLeastRecent.slice(
            0,
            Math.max(1, Math.ceil(byLeastRecent.length * 0.3)),
          );
          selectedRecipe = pickFromPool(window);
        }

        if (!selectedRecipe) {
          // Final safety: if everything above still returns null, fall back to
          // any recipe in the calorie-filtered pool. This should be unreachable.
          selectedRecipe =
            calorieFilteredRecipes[
              Math.floor(Math.random() * calorieFilteredRecipes.length)
            ];
        }

        // Use existing recipe image or generate unique AI image
        const recipeDescription =
          selectedRecipe.description || `Delicious ${mealType} meal`;
        ...
        if (!imageUrl) {
          try {
            console.log(
              `[Meal Plan Generator] Generating unique AI image for: ${selectedRecipe.name}`,
            );
            ...
            const recipeForImage = {
              ...
              estimatedNutrition: {
                ...
                protein: parseFloat(
                  selectedRecipe.proteinGrams?.toString() || "0",
                ),
            ...
            console.log(
              `[Meal Plan Generator] Successfully generated image: ${imageUrl.substring(0, 50)}...`,
            );
          } catch (error) {
            console.error(
              `[Meal Plan Generator] Failed to generate AI image for ${selectedRecipe.name}:`,
              error,
            );
    ...
    if (generateMealPrep !== false) {
      mealPlan.startOfWeekMealPrep =
        this.generateMealPrepInstructions(mealPlan);
    }
  ...
  private selectRecipeWithIngredientLimit(
    ...
    // Phase B+C (2026-04-13): this scorer used to silently return the same
    // recipe repeatedly because it rewarded ingredient reuse without any
    // plan-level dedup. We now hard-exclude already-used recipes from the
    // primary pool and only reuse when the pool is exhausted. Variety
    // scoring prefers recipes whose mainIngredientTags are not already
    // dominant in the plan.
    const usedRecipeIds = new Set<string>(
      currentMeals.map((m: any) => m.recipe.id),
    );
    ...
    // Main ingredient tag occurrence across the plan so far
    const mainTagCounts = new Map<string, number>();
    ...
    // Score recipes based on ingredient reuse AND main-tag variety
    const scoreRecipe = (recipe: any) => {
      ...
      recipeIngredients.forEach((ingredient: any) => {
        ...
        if (usedIngredients.has(ingredientName)) {
          reuseScore += 2; // Bonus for shared pantry ingredients
        } else {
      ...
      // Variety penalty: push down recipes whose main tag is already saturated
      let varietyPenalty = 0;
      const mainTags: string[] = (recipe.mainIngredientTags || []).map(
        (t: string) => String(t).toLowerCase(),
      );
      for (const tag of mainTags) {
        const count = mainTagCounts.get(tag) || 0;
        varietyPenalty += count * 5;
      }

      return {
        recipe,
        score: reuseScore - newIngredientsCount - penalty - varietyPenalty,
        newIngredientsCount,
      };
    };

    // Primary pool: recipes that have NOT been used yet
    ...
    const scoredRecipes = poolToScore.map(scoreRecipe);

    // Filter out recipes that would exceed the ingredient limit (unless none fit)
    let validRecipes = scoredRecipes.filter(
      (item) => item.newIngredientsCount <= remainingIngredientSlots,
    );
    if (validRecipes.length === 0) {
      validRecipes = scoredRecipes;
    }

    // Sort by score (highest first), take top 30% window, random-pick for variety
  ...
  private generatePrepSteps(shoppingList: any[]): any[] {
    ...
    steps.push({
      step: stepNumber++,
      instruction:
        "Label and store all prepped ingredients according to storage instructions. Clean prep area and wash containers.",
      estimatedTime: 10,
      ingredients: [],
    });
