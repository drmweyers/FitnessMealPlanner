...
-- Migration: 0014_optimize_customer_visibility_indexes (No Transaction Version)
-- Date: 2025-09-02
-- Purpose: Add optimized indexes for customer visibility queries and general performance
-- Note: CONCURRENTLY indexes cannot run in transactions, so this version omits BEGIN/COMMIT
...
-- 11. Text search optimization improvements

-- Add trigram extension for fuzzy text search (if not exists)
-- Note: Extensions require superuser privileges, may need to be added separately
-- CREATE EXTENSION IF NOT EXISTS pg_trgm;

-- Update table statistics for query planner optimization
-- Note: These need to be run separately as they are not DDL statements
-- ANALYZE users;
-- ANALYZE recipes;  
