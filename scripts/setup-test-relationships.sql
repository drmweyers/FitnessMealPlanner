...
BEGIN
  ...
  IF trainer_uuid IS NOT NULL AND customer_uuid IS NOT NULL THEN
    -- Insert invitation relationship (idempotent)
    ...
    VALUES (
      ...
      NOW() + INTERVAL '365 days',  -- Valid for 1 year (test data)
      NOW() - INTERVAL '1 day',      -- Accepted 1 day ago
      NOW() - INTERVAL '7 days'      -- Invitation sent 7 days ago
    )
    ON CONFLICT (token) DO NOTHING;

    RAISE NOTICE 'Test relationship created: trainer.test@evofitmeals.com -> customer.test@evofitmeals.com';
  ELSE
    RAISE WARNING 'Test accounts not found. Please run seed-test-accounts.js first.';
  END IF;
END $$;
...
-- Show all test account relationships
SELECT
