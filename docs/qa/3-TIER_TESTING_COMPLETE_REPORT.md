...
test("should return 5 meal types for Starter tier", async () => {
  const starterMealTypes = mockMealTypes.filter(
    (mt) => mt.tierLevel === "starter",
  );
...
test("should enable white-label mode", async () => {
  ...
  const result = await service.enableWhiteLabel(
    mockTrainerId,
    true,
    mockRequest,
  );
...
test("Starter tier should see 5 meal types in dropdown", async ({ page }) => {
  ...
  const accessibleTypes = await page
    .locator('[data-testid="meal-type-option"]:not([data-locked="true"])')
    .count();
  expect(accessibleTypes).toBe(5);

  // Verify lock icons appear on inaccessible types
  const lockedTypes = await page
    .locator('[data-testid="meal-type-option"][data-locked="true"]')
    .count();
